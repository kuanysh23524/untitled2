...
def translatePercentage1(*percentages):
    return [translateLetter1('A+', 'D', 'C-', 'A-')[int((percentage - 45) / 5)] for percentage in percentages]

def calculateGPA1(*args):
    points = args[::2]
    credits = args[1::2]
...
def calculate_and_save_gpas():
    credits = read_grades("credits.txt")
    overall_gpas = []

    with open(f"overallGPAs.txt", 'w') as result_file:
        for course in ['math', 'chemistry', 'english']:
            scores = read_grades("{course}.txt")
            points = translatePercentage1(*scores)
            gpa = calculateGPA1(*points, *credits)
...
class Student:
    def __init__(self, name, num_courses, scores):
        ...
        self.scores = scores
        self.overall_gpa = self.calculateGPA2()
        self.status = self.setStatus()

    def calculateGPA2(self):
